...
t_values = [int(t.text) for t in root.findall('.//forecast/T') if t.text is not None]
f_values = [int(f.text) for f in root.findall('.//forecast/F') if f.text is not None]  # Added for wind speed
